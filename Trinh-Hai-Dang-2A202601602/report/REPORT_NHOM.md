...
1. [Lựa chọn tài liệu](#1-lựa-chọn-tài-liệu-document-set-quality--nhóm-10-điểm) (10đ)
2. [Thiết kế chiến lược](#2-thiết-kế-chiến-lược-strategy-design--nhóm-15-điểm) (15đ)
3. [Câu hỏi đánh giá & Chất lượng truy xuất](#3-câu-hỏi-đánh-giá--chất-lượng-truy-xuất-retrieval-quality--nhóm-10-điểm) (10đ)
4. [Thuyết trình & Bài học nhóm](#4-thuyết-trình-demo--bài-học-nhóm--nhóm-5-điểm) (5đ)
5. [Tự đánh giá](#tự-đánh-giá-phần-nhóm)
...
> Chính sách vận hành sàn Shopee nhìn từ **hai phía** — nghĩa vụ của người mua (đổi trả, hoàn phí, giao hàng, thanh toán) và nghĩa vụ của người bán (đăng bán, hàng cấm, phí dịch vụ). Chọn hai phía có chủ đích để trường `customer_role` thực sự phân biệt được tài liệu — nếu không thì câu hỏi có filter sẽ không chứng minh được gì.

### Danh sách tài liệu (Data Inventory)

Corpus dùng chung: `data/k4_ecommerce/` — **10 tài liệu**, toàn bộ lấy từ trang trợ giúp công khai của Shopee, ngày lấy `2026-08-03`.
...
**Kết quả CHECKPOINT 2** (`python scripts/kiem_tra_corpus.py`, đã chạy lại độc lập ngày 03/08/2026):
...
|---|---|---|---|
| `doc_id` | str | `return-shipping-fee` | Khoá truy vết: mọi chunk đều mang `doc_id` của file gốc, nhờ đó `delete_document()` xoá đúng và câu trả lời truy vết về đúng nguồn |
...
Chạy `ChunkingStrategyComparator().compare(text, chunk_size=400)` trên 3 tài liệu đại diện (đã bỏ front matter, chỉ đo phần thân):

**`return-refund-policy` (1.490 ký tự)**

| Chiến lược | Số chunk | Độ dài TB | Giữ được ngữ cảnh không? |
|---|---:|---:|---|
| `fixed_size` | 5 | 330,0 | Không — cắt giữa câu, mốc "15 ngày" và điều kiện đi kèm bị tách |
| `by_sentences` | 4 | 370,3 | Có — mỗi chunk trọn câu |
| `recursive` | 7 | 211,3 | Có — cắt ở ranh giới đoạn trước |

**`restricted-products-policy` (1.189 ký tự)**

| Chiến lược | Số chunk | Độ dài TB | Giữ được ngữ cảnh không? |
|---|---:|---:|---|
| `fixed_size` | 4 | 327,3 | Không — danh sách 20 nhóm hàng cấm bị cắt ngang |
| `by_sentences` | 2 | 593,0 | Kém — 2 chunk quá to, gộp nhiều nhóm hàng vào một vector |
| `recursive` | 4 | 296,3 | Trung bình |

**`shipping-fee-discount-program` (1.113 ký tự)**

| Chiến lược | Số chunk | Độ dài TB | Giữ được ngữ cảnh không? |
|---|---:|---:|---|
| `fixed_size` | 3 | 397,7 | Không |
...
- **✅ Đã tự verify độc lập (Đăng, 03/08/2026):** chạy lại từ đầu với `src/` và `data/k4_ecommerce/` thật của nhóm (không phải suy diễn từ báo cáo) bằng `LocalEmbedder` thật: kết quả **32 chunk, thứ hạng #1 #1 #1 trượt #1 — khớp chính xác 100%** với số liệu nhóm đã ghi ở trên. Điểm agent đo lại được 3/5 (thay vì 2/5) — chênh lệch này đến từ khác biệt trong cách viết stub trích xuất câu trả lời (LLM giả lập) giữa lần chạy của Đăng và bản `bench_ca_nhom.py` gốc của nhóm, **không phải do sai khác ở `FixedSizeChunker`, `EmbeddingStore` hay dữ liệu** — retrieval (phần được chấm điểm chính) đã được xác nhận khớp tuyệt đối.
...
- ⚠️ *Ghi chú của Đăng:* có một lần chạy lại khác của Linh cho kết quả 10/10 (agent 5/5), nhưng lần chạy đó dùng `mock embeddings fallback` (thiếu `sentence-transformers`) kèm một lớp "hybrid lexical scoring" tự thêm vào `EmbeddingStore` — tức đổi cả **embedder lẫn thuật toán chấm điểm**, không chỉ đổi chunker. Vì mục đích của Mục 2 là so sánh **các chiến lược chunking trên cùng một điều kiện** (cùng embedder thật, cùng logic store), kết quả 10/10 đó không được đưa vào bảng so sánh chính thức — số liệu 4/10 ở trên (đo bằng `LocalEmbedder` thật, cùng điều kiện với 4 thành viên còn lại) mới là số liệu dùng để so sánh công bằng. Đây là bài học quan trọng: đổi nhiều biến cùng lúc (embedder + scoring) sẽ làm mất giá trị của một so sánh có kiểm soát, dù kết quả cuối "đẹp" hơn.
...
Toàn bộ số dưới đây lấy từ `python scripts/bench_ca_nhom.py`, chạy đúng gói `src/` của từng người trên cùng corpus và cùng 5 câu hỏi, cùng `EMBEDDING_PROVIDER=local`. Số trong cột Q là **thứ hạng của chunk chứa gold answer**. Dòng **Trịnh Hải Đăng** đã được Đăng tự chạy lại độc lập ngày 03/08/2026 và xác nhận khớp số chunk + thứ hạng; dòng **Đỗ Văn Linh** giữ nguyên số liệu đo cùng điều kiện với 4 người còn lại (xem ghi chú ở mục "Thành viên 4" về lần chạy khác dùng mock + lexical scoring, không đưa vào bảng vì đổi nhiều biến cùng lúc).
...
| Trịnh Hải Đăng *(tự verify độc lập)* | `FixedSizeChunker(500, 50)` | 32 | #1 | **#1** | #1 | trượt | #1 | 2/5 | **6/10** |
...
| Trịnh Hải Đăng | Ít chunk nhất (32), rẻ nhất về chi phí nhúng; **số liệu đã được tự verify độc lập khớp 100%** | Mỗi chunk chứa nhiều chủ đề nên vector bị trung bình hoá; agent kém |
...
>
> Điều nhóm tự tin kết luận là **cơ chế**: chiến lược thắng ở Q4 vì nó cắt **danh sách liệt kê theo từng mục**. Khi 20 nhóm hàng cấm nằm chung một chunk, vector của chunk đó là trung bình của 20 chủ đề nên không khớp riêng "đồ cổ"; tách mỗi mục thành một chunk thì dòng đó mới có vector riêng. **Bốn thành viên còn lại đều trượt Q4** trong điều kiện đo giống nhau (cùng embedder thật, cùng logic store) — đây là bằng chứng cơ chế mạnh nhất của cả báo cáo.
>
...
> 3. **So sánh chỉ có giá trị khi chỉ đổi một biến.** Lần chạy khác của Linh (mock + hybrid lexical scoring) đạt 10/10, nhưng vì đổi cả embedder lẫn thuật toán chấm điểm cùng lúc, kết quả đó không so sánh được với 4 dòng còn lại — nhóm không dùng nó để kết luận chiến lược nào tốt hơn.
>
...
Mỗi `gold_snippet` là cụm trích **nguyên văn** từ corpus, đã được `grep` xác minh **chỉ khớp đúng 1 tài liệu**, nên chấm được tự động ở **mức chunk**.
...
2. **Ngữ cảnh tiêu đề quan trọng hơn việc chọn kiểu chunker.** Bỏ tiền tố tiêu đề khiến cùng một chunker rơi từ 8/10 xuống 4/10 — chênh lệch lớn hơn nhiều so với khoảng cách giữa các chiến lược khác nhau (8 vs 7 vs 6).
3. **Mịn hơn không phải tốt hơn.** `RecursiveChunker(120)` mịn nhất (143 chunk) nhưng tệ nhất (1/10) trong lần quét 12 cấu hình. Cái quyết định là **cắt có tôn trọng ranh giới ngữ nghĩa hay không**, không phải kích thước.

**Bài học rút ra khi so sánh trong nhóm:**

> Cùng corpus, cùng 5 câu hỏi, chỉ khác chiến lược chunking mà chênh nhau tới **7 điểm** (từ 1/10 đến 8/10, tính cả lần quét 12 cấu hình). Khác biệt không nằm ở "chunk to hay nhỏ" mà ở **chunk có khớp đơn vị ngữ nghĩa của văn bản hay không**: với văn bản chính sách, đơn vị đó là **điều/khoản**, và danh sách liệt kê phải được cắt theo **từng mục**.
>
...
>
> Bài học thứ ba, rút ra khi Đăng verify độc lập phần của mình: **một so sánh chỉ đáng tin khi chỉ đổi đúng một biến.** Lần chạy khác của Linh (đổi cả embedder lẫn thuật toán chấm điểm) cho điểm cao hơn hẳn nhưng không dùng để kết luận được gì về chunking — bài học này áp dụng ngược lại cho chính cách nhóm đo đạc: mọi so sánh trong Mục 2 chỉ có giá trị vì cả 5 dòng chính thức đều dùng chung `LocalEmbedder` thật và chung logic chấm điểm.
...
Câu 5 cần lọc `{"customer_role": ["buyer", "both"]}` vì tài liệu gold có vai `both` — lọc cứng `"buyer"` sẽ **mất luôn tài liệu chứa đáp án**. Các store chỉ so khớp bằng `==` sẽ gặp lỗi `"both" == ["buyer","both"]` cho `False` ở mọi bản ghi → **trả về rỗng**, không báo lỗi gì. Sau khi sửa, `EmbeddingStore._matches()` chung của nhóm đã hỗ trợ list filter (khớp nếu thuộc tập) và không còn cảnh báo.
...
| Thuyết trình (Demo) | chưa chấm *(buổi demo chưa diễn ra tại thời điểm viết báo cáo)* |
| **Tổng phần nhóm (tạm tính, chưa gồm Demo)** | **31 / 35** |

**Căn cứ tự chấm:**

- *Lựa chọn tài liệu — 9/10:* đạt CHECKPOINT 2 sạch (đã verify lại độc lập, 0 lỗi chặn), 10 tài liệu nguồn công khai có provenance đầy đủ, `customer_role` đủ 3 giá trị (4 buyer / 3 seller / 3 both) nên filter thật sự lọc được. Tự trừ 1 vì corpus chỉ từ **một sàn (Shopee)** — đa dạng nguồn còn hạn chế, và cả 3 tài liệu `returns` đều vai `buyer`.
- *Thiết kế chiến lược — 14/15:* 5 thành viên dùng **5 chiến lược khác nhau** (custom theo điều/khoản, recursive, fixed 500/50, fixed 250/100, sentence), đo trên **cùng corpus + cùng query + cùng embedder thật**, có bảng so sánh đầy đủ và ablation chứng minh cơ chế; dòng Đăng đã được chính chủ tự verify lại độc lập và khớp 100% số chunk/thứ hạng.
- *Chất lượng truy xuất — 8/10:* điểm cao nhất nhóm đo thật bằng `python scripts/bench_ca_nhom.py` (2+1+2+1+2). Điểm 5 người: 8 / 7 / 6 / 5 / 4.

> **Ghi chú:** lần chạy 10/10 (mock + hybrid lexical scoring) của Linh không đưa vào bảng so sánh chính thức ở Mục 2 vì lý do đã nêu ở đó (đổi cả embedder lẫn thuật toán chấm điểm, không còn là so sánh chỉ-đổi-chunker), chỉ giữ lại như một quan sát phụ.
